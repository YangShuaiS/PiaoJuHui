...
- (void)setupToolbars
{
    ...
    indexLabel.textColor = RGB(0xff, 0xff, 0xff);
    indexLabel.font = [UIFont boldSystemFontOfSize:16];
    indexLabel.backgroundColor = RGBA(0x02, 0x02, 0x03, 0.3);
...
//    top.backgroundColor = RGBA(0x02, 0x02, 0x03, 0.3);
//    top.frame = CGRectMake(0, 0, WIDTH, NavHeight);
//    [self addSubview:top];
    // 2.删除按钮
//    UIButton *saveButton = [[UIButton alloc] init];
//    [saveButton setImage:UIIMAGE("TWDT_005_icon_删除") forState:UIControlStateNormal];
//    saveButton.clipsToBounds = YES;
//    [saveButton addTarget:self action:@selector(saveImage) forControlEvents:UIControlEventTouchUpInside];
//    _saveButton = saveButton;
//    [top addSubview:saveButton];
    
    _indexLabel.frame =  CGRectMake(0, self.bounds.size.height- TabBarHeight, WIDTH, TabBarHeight);

//    _saveButton.frame = CGRectMake(kScreenWidth-28-20, top.frame.size.height-10-20, 20, 20);
}
...
- (void)setIma:(UIImage *)ima{
    ...
    _leftBrowserImgView.showImg.image = ima;
}
/**数据刷新*/
- (void)refreshData {
    [self initScrollContSize];
